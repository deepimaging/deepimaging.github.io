...
random_mask = np.random.normal(size=(208, 176))
def GaussianSampleLayer(x):
  ...
  # image plane;
  transfer_func = random_mask
  transfer_func = np.where(transfer_func > np.percentile(transfer_func, 50), 1, 0)
  transfer_func = transfer_func[:,:, np.newaxis]
  x = tf.complex(x, tf.zeros_like(x))
  masked_x = tf.signal.fftshift(tf.signal.fft2d(x, name='nmsl5'), axes=(1,2), name='nmsl6') * transfer_func
  masked_x = tf.abs(tf.signal.ifft2d(tf.signal.ifftshift(masked_x, axes=(1,2), name='nmsl7')))
  return masked_x
...
x = Lambda(lambda x1: GaussianSampleLayer(x1))(x_in)
y = kfjNet(x)
model2 = Model(inputs=x_in, outputs=y)
optimizer = Adam(lr=0.001)
callbacks = [
            ModelCheckpoint(
                path.join('D:/桌面文件2/出国准备/Duke/BME590OpticsImaging/final_project/', "weights.random_mask.best.h5"),
...
model2.save_weights("model_random_mask.h5")
