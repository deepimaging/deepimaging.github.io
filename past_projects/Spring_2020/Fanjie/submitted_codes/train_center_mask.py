...
def CircleShapeLayer(x):
  ...
  # image plane;
  transfer_func = np.zeros((100, 100))
  radius = 40
  for i in range(100):
      for j in range(100):
          if ((i-100/2)**2 + (j-100/2)**2) <= radius**2:
              transfer_func[i, j] = 1
  transfer_func = np.resize(transfer_func, (208, 176))
  transfer_func = np.reshape(transfer_func, (208, 176, 1))
...
x = Lambda(lambda x1: CircleShapeLayer(x1))(x_in)
y = kfjNet(x)
model2 = Model(inputs=x_in, outputs=y)
optimizer = Adam(lr=0.001)
callbacks = [
            ModelCheckpoint(
                path.join('D:/桌面文件2/出国准备/Duke/BME590OpticsImaging/final_project/', "weights.center_mask.best.h5"),
...
model2.save_weights("model_center_mask.h5")
