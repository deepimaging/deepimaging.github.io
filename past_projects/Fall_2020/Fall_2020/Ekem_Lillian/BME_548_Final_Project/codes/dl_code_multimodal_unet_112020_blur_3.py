...
data_x_train = data_cropped_train[:,:,:,0:3]
data_x_train = np.rot90(data_x_train,axes=(0,2))
data_x_train = np.rot90(data_x_train,axes=(1,2))
data_x_train = np.reshape(data_x_train,[len(data_x_train),448,512,3])
...
data_x_test = data_cropped_test[:,:,:,0:3]
data_x_test = np.rot90(data_x_test,axes=(0,2))
data_x_test = np.rot90(data_x_test,axes=(1,2))
data_x_test = np.reshape(data_x_test,[len(data_x_test),448,512,3])
...
data_x_train_blur = np.empty([len(data_x_train),448,512,3])
for i in range(data_x_train.shape[0]): 
    for j in range(3):
        blur_train = cv2.GaussianBlur(data_x_train[i,:,:,j], (3,3), 0)
        data_x_train_blur[i, :, :, j] = blur_train

data_x_test_blur = np.empty([len(data_x_test),448,512,3])
for i in range(data_x_test.shape[0]): 
    for j in range(3):
        blur_test = cv2.GaussianBlur(data_x_test[i,:,:,j], (3,3), 0)
        data_x_test_blur[i, :, :, j] = blur_test




x_train = np.copy(data_x_train_blur)
y_train = np.copy(data_y_train)

x_val = np.copy(data_x_test_blur)
y_val= np.copy(data_y_test)


def cnn(fliter_num,kernel_size):
    input_layer = keras.layers.Input(shape=(448, 512, 3))
    ...
    np.save('/home/alex/Stroke/results/train_acc_mm_blur_3.npy',train_acc)
    val_acc = history.history['val_dice_metric']
    np.save('/home/alex/Stroke/results/val_acc_mm_blur_3.npy',val_acc)

    model.save('/home/alex/Stroke/results/model_blur_3.h5')
